...
function stageSelectColumns() {
  return "stage_id,stage_no,stage_name,unlock_stage_no,difficulty,stage_category,clear_condition_type,clear_condition_params,recommended_power,stamina_cost,is_active,published_at,unpublished_at,created_at,updated_at";
}
...
export async function listStages() {
  ...
  const { data, error } = await supabase
    .schema("master")
    .from("m_stage")
    .select(stageSelectColumns())
    .order("stage_no", { ascending: true });

  if (error) throw new Error(error.message);

  const rows = (data ?? []) as unknown[];
  return rows.map((row) => mapStageRow(row as Record<string, unknown>));
}
...
export async function listPieceOptions() {
  const supabase = getSupabaseAdminClient();
  const { data, error } = await supabase
    .schema("master")
    .from("m_piece")
...
export async function existsStageNo(stageNo: number) {
  const supabase = getSupabaseAdminClient();
...
export async function insertStage(input: StageFormInput) {
  const supabase = getSupabaseAdminClient();
